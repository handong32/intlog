#! /bin/bash

currdate=`date +%m_%d_%Y_%H_%M_%S`

export NITERS=${NITERS:='2'}
export BEGIN_ITER=${BEGIN_ITER:='0'}
export MDVFS=${MDVFS:="0x1d00 0x1c00 0x1b00 0x1a00 0x1900 0x1800 0x1700 0x1600 0x1500 0x1400 0x1300 0x1200 0x1100 0x1000 0xf00 0xe00 0xd00 0xc00"}
export MQPS=${MQPS:='200000 400000 600000'}
export ITRS=${ITRS:-"2 4 10 20 30 40 50 100 200 300 400"}
export MRAPL=${MRAPL:-"135 75 55"}

function runSiloBench
{
    timeout 600 python3 -u silo_bench.py "$@"
}

function run
{
    echo "DVFS ${MDVFS}"
    echo "ITRS ${ITRS}"
    echo "MQPS ${MQPS}"
    echo "MRAPL ${MRAPL}"
    echo "NITERS ${NITERS}"
    echo "mkdir ${currdate}"
    mkdir ${currdate}    
     
    for dvfs in ${MDVFS}; do
	for itr in $ITRS; do	    
	    for qps in ${MQPS}; do	    	    
		for r in ${MRAPL}; do				
		    for i in `seq ${BEGIN_ITER} 1 $NITERS`; do
		        runSiloBench --qps ${qps} --time 20 --itr ${itr} --rapl ${r} --dvfs ${dvfs} --nrepeat ${i}
			sleep 1
			mv linux.mcdsilo.* ${currdate}/
			sleep 1
			echo "FINISHED: runSiloBench --qps ${qps} --time 20 --itr ${itr} --rapl ${r} --dvfs ${dvfs} --nrepeat ${i}"
		    done
		done
	    done
	done
    done
}

"$@"

